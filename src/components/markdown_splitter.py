...
class MarkdownDocumentSplitter:
    '''
    Chunks Markdown by sentence.

    This remembers the section that a vector should belong to and stores it
    as metadata when outputting the documents. Additionally, the run length
    of the section is tracked, so a single vector can be used to reconstruct
    the text from the position of the vector, the end of the section.
    '''
        ...
        for doc in documents:
            ...
            for x in range(num_sections):
                ...
                for span in sentence_spans:
                    ...
                    output.append(
                        Document(
                            content=chunk_data["content"], meta=chunk_data["metadata"]
                        )
                    )
